...
class FakeWorker {
  ...
  scriptedResponses: Record<string, string[]> = {
    uci: ['id name Stockfish', 'id author the Stockfish developers', 'option name Threads type spin default 1 min 1 max 1024', 'uciok'],
    isready: ['readyok']
  }
...
describe('engine module', () => {
  it('parses UCI info and bestmove lines correctly', () => {
    const info = parseInfoLine('info depth 8 nodes 1234 nps 5678 score cp 45 pv e2e4 e7e5')
  ...
  it('initializes engine and executes search via fake worker', async () => {
    ...
    const res = await engine.search('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', [], { depth: 4 })
