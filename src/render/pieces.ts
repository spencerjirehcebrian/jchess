...
export class PieceManager {
  ...
  private precacheGeometries(theme: Theme) {
    ...
    for (const role of roles) {
      const whiteMesh = meshPiece(PIECE_DEFINITIONS[role], theme.white, 'white')
      const blackMesh = meshPiece(PIECE_DEFINITIONS[role], theme.black, 'black')
  ...
  updatePosition(pos: Position, boardFlipped: boolean) {
    ...
    const getMaterial = () => {
      if (!materialCache.has('mat')) {
        materialCache.set('mat', new THREE.MeshLambertMaterial({ vertexColors: true }))
      }
    ...
    currentPieces.forEach(({ role, color }, sq) => {
      ...
      for (const [id, active] of this.activePieces.entries()) {
        if (!usedIds.has(id) && active.role === role && active.color === color && active.square === sq) {
          matchedPiece = active;
          break
        }
      }
      ...
      if (!matchedPiece) {
        for (const [id, active] of this.activePieces.entries()) {
          if (!usedIds.has(id) && active.role === role && active.color === color) {
            matchedPiece = active
            break
          }
        }
