...
export function DifficultyPicker({ controller }: DifficultyPickerProps) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 'var(--size-xs)', fontFamily: 'var(--font-display)', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 'var(--sp-1)' }}>
        Engine Level
      </div>
        ...
        return (
          <button
            ...
            style={{
              ...
              color: disabled ? 'var(--text-faint)' : isSelected ? 'var(--accent-bright)' : 'var(--text-dim)',
          ...
          >
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--size-sm)', opacity: 0.8 }}>
              ~{lvl.approxElo} Elo
            </span>
          </button>
        )
      ...
      {!caps.threaded && (
        <div style={{ fontSize: 'var(--size-xs)', color: 'var(--text-faint)', marginTop: 'var(--sp-2)', lineHeight: 1.3 }}>
          Running in single-thread mode. Levels 7 and 8 need a secure connection.
        </div>
      )}
    </div>
  )
