...
export function MoveList({ controller }: MoveListProps) {
  ...
  const pairs: { moveNum: number; white: { san: string; index: number } | null; black: { san: string; index: number } | null }[] = []
  ...
  return (
    ...
    >
      {pairs.length === 0 ? (
        <div style={{ color: 'var(--text-faint)', fontStyle: 'italic' }}>No moves played yet</div>
      ) : (
        pairs.map((p) => {
          ...
          return (
            <div key={p.moveNum} style={{ display: 'grid', gridTemplateColumns: '40px 1fr 1fr', padding: '2px 0', alignItems: 'center' }}>
              ...
              {p.white && (
                <button
                  ...
                  style={{
                    ...
                    borderLeft: isWhiteActive ? '2px solid var(--accent)' : '2px solid transparent',
              ...
              {p.black ? (
                <button
                  ...
                  style={{
                    ...
                    borderLeft: isBlackActive ? '2px solid var(--accent)' : '2px solid transparent',
                    paddingLeft: 'var(--sp-1)',
                    cursor: 'pointer'
                  }}
                >
