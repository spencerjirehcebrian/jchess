...
export function serializePgn(state: GameState): string {
  const dateStr = new Date(state.startedAt).toISOString().slice(0, 10).replace(/-/g, '.')
  const resultStr = state.status.kind === 'over' ? formatResult(state.status.result) : '*'

  const whiteName = state.humanColor === 'white' ? 'Player' : `Stockfish (Level ${state.difficulty})`
  const blackName = state.humanColor === 'black' ? 'Player' : `Stockfish (Level ${state.difficulty})`

  const tags: [string, string][] = [
    ['Event', 'Voxel Chess'],
    ['Site', typeof window !== 'undefined' ? window.location.hostname : 'localhost'],
    ['Date', dateStr],
    ['Round', '-'],
    ['White', whiteName],
    ['Black', blackName],
  ...
  if (state.initialFen !== FIXTURE_FENS.START) {
...
export function parsePgn(pgnText: string): ParsedPgn {
  ...
  const fullMoveText = moveTextLines.join(' ').replace(/\{[^}]*\}/g, '').replace(/\([^)]*\)/g, '')
