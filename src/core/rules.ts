...
import { Square, Move, Color, Role, HistoryEntry, Result, squareToName, nameToSquare } from './types'
...
export function legalMovesFrom(pos: Position, from: Square): Move[] {
  ...
  for (const to of dests) {
    ...
    if (isPawn && toRank === lastRank) {
      const promotions: Exclude<Role, 'pawn' | 'king'>[] = ['queen', 'rook', 'bishop', 'knight']
...
export function toUci(move: Move): string {
  ...
  const promo = move.promotion ? (move.promotion === 'knight' ? 'n' : move.promotion[0]!) : ''
...
export function fromUci(uciStr: string): Move | null {
  ...
  const to = nameToSquare(uciStr.slice(2, 4))
...
export function outcome(pos: Position, history: HistoryEntry[]): Result | null {
  ...
  const extractKey = (fenString: string) => fenString.split(' ').slice(0, 4).join(' ')
